...
CLOSURE_STATUSES = ("review", "published", "archived")
SEE_ALSO_RE = re.compile(r"^#{2,}\s+See Also\s*$", re.IGNORECASE)
H2_RE = re.compile(r"^#{1,2}\s+\S")


def lint_body_wikilink_closure(root: Path, articles: list[Path]) -> list[str]:
    """Deterministic closure core (AGENTS.md §498-507, §720).

    At status review/published/archived a project artifact's body prose must
    contain no `[[wikilinks]]`. They are permitted only inside the `## See Also`
    section or in frontmatter. This is the mechanical publish blocker; the LLM
    layer in workspace-align-closure.prompt.md never relaxes it.
    """
    findings: list[str] = []
    for article in articles:
        rel_str = str(article.relative_to(root)).replace("\\", "/")
        if "workspace-projects" not in rel_str:
            continue
        fm = parse_frontmatter(article)
        ...
        for line in body.splitlines():
            if SEE_ALSO_RE.match(line):
                in_see_also = True
                continue
            if in_see_also and H2_RE.match(line) and not SEE_ALSO_RE.match(line):
                in_see_also = False  # a later heading ends the See Also section
            if in_see_also:
                continue  # wikilinks allowed inside See Also
            for match in WIKILINK_RE.finditer(line):
                findings.append(
                    f"{rel_str}: body wikilink [[{match.group(1).strip()}]] "
                    f"at status {status} (closure rule; move to ## See Also or frontmatter)"
                )
    return findings
...
def run_lint(root: Path, scope: Path | None) -> dict:
    ...
    for key, items in extra.items():
        findings[key] = items
    findings["body_wikilink_closure"] = lint_body_wikilink_closure(root, articles)
...
CLOSURE_ONLY_CHECKS = ("body_wikilink_closure",)
...
def main() -> int:
    ...
    parser.add_argument(
        "--closure-only",
        action="store_true",
        help="Exit non-zero only on closure violations (publish-gate blocker). "
        "Full report is still written.",
    )
    ...
    sev = severity_map()
    blocking = set(CLOSURE_ONLY_CHECKS) if args.closure_only else None
    err_count = sum(
        len(v)
        for k, v in findings.items()
        if sev.get(k) == "error" and (blocking is None or k in blocking)
    )
    print(f"Report: {report}")
    print(f"{'Closure violations' if args.closure_only else 'Errors'}: {err_count}")
    for check, items in findings.items():
        if items and (blocking is None or check in blocking):
