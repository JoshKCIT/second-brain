---
source_url: https://docs.snowflake.com/en/user-guide/tables-iceberg-catalog
vendor: snowflake
topic: catalog-integrations
...
With the Snowflake Catalog SDK, you can query [Iceberg tables](https://docs.snowflake.com/user-guide/tables-iceberg) using a third-party engine such as Apache Spark™ or Trino.
...
Before you can use the Snowflake Catalog SDK, you need a Snowflake database with one or more Iceberg tables. To create an Iceberg table, see [Create an Apache Iceberg™ table in Snowflake](https://docs.snowflake.com/user-guide/tables-iceberg-create).
...
> [!note] Note
> You can use any Snowflake-supported [JDBC driver connection parameter](https://docs.snowflake.com/developer-guide//jdbc/jdbc-parameters) in your configuration by using the following syntax: `--conf spark.sql.catalog.snowflake_catalog.jdbc.*property-name*=*property-value*`
...
> [!note] Note
> If you receive vectorized read errors while running queries, you can disable the vectorized reads for your session by configuring: `spark.sql.iceberg.vectorization.enabled=false`. To keep using vectorized reads, you can set the [STORAGE\_SERIALIZATION\_POLICY](https://docs.snowflake.com/sql-reference/parameters#label-storage-serialization-policy) parameter.
