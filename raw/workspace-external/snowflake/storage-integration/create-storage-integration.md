...
vendor: snowflake
topic: storage-integration
fetched_at: 2026-06-10T06:40:02Z
revalidate_after: 2026-07-10T06:40:02Z
...
A single storage integration can support multiple external stages. The URL in the stage definition must align with the storage location specified for the STORAGE\_ALLOWED\_LOCATIONS parameter.
...
> 	Snowflake charges a per-byte fee when you unload data from Snowflake into an external stage in a different [region](https://docs.snowflake.com/user-guide/intro-regions) or different cloud provider. For details, see the [pricing page](https://www.snowflake.com/pricing/).
...
[ALTER STORAGE INTEGRATION](https://docs.snowflake.com/sql-reference/sql/alter-storage-integration), [DROP INTEGRATION](https://docs.snowflake.com/sql-reference/sql/drop-integration), [SHOW INTEGRATIONS](https://docs.snowflake.com/sql-reference/sql/show-integrations)
...
- `POSTGRES_EXTERNAL_STORAGE`: Creates a storage integration for use with [Snowflake Postgres](https://docs.snowflake.com/user-guide/snowflake-postgres/postgres-pg_lake). Only one storage location is allowed for this type of integration.
- `POSTGRES_INTERNAL_STORAGE`: Creates a storage integration for use with the managed storage associated with a Snowflake Postgres instance. For full syntax, see [CREATE STORAGE INTEGRATION (Postgres Internal Storage)](https://docs.snowflake.com/sql-reference/sql/create-storage-integration-postgres-internal).
...
Explicitly prohibits external stages that use the integration from referencing one or more storage locations (i.e. S3 buckets or GCS buckets). Supports a comma-separated list of URLs for existing storage locations and, optionally, paths used to store data files for loading/unloading. Commonly used when STORAGE\_ALLOWED\_LOCATIONS is set to the `*` wildcard, allowing access to all buckets in your account except for blocked storage locations and, optionally, paths.
...
> `STORAGE_AWS_ROLE_ARN = '*iam_role*'`
> 
> Specifies the Amazon Resource Name (ARN) of the AWS identity and access management (IAM) role that grants privileges on the S3 bucket containing your data files. For more information, see [Configuring secure access to Amazon S3](https://docs.snowflake.com/user-guide/data-load-s3-config).
...
> 
> When users unload Snowflake table data to data files in an S3 stage using [COPY INTO <location>](https://docs.snowflake.com/sql-reference/sql/copy-into-location), the unload operation applies an ACL to the unloaded data files. The data files apply the `"s3:x-amz-acl":"bucket-owner-full-control"` privilege to the files, granting the S3 bucket owner full control over them.
> 
> `USE_PRIVATELINK_ENDPOINT = { TRUE | FALSE }`
> 
> Specifies whether to use outbound private connectivity to harden your security posture. For information about using this parameter, see [Private connectivity to external stages for Amazon Web Services](https://docs.snowflake.com/user-guide/data-load-aws-private).
...
> `USE_PRIVATELINK_ENDPOINT = { TRUE | FALSE }`
> 
> Specifies whether to use outbound private connectivity to harden your security posture. For information about using this parameter, see [Private connectivity to external stages and Snowpipe automation for Microsoft Azure](https://docs.snowflake.com/user-guide/data-load-azure-private).
...
> 
> If you must recreate a storage integration after it has been linked to one or more stages, you must reestablish the association between each stage and the storage integration by executing [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage) `*stage_name*` SET STORAGE\_INTEGRATION = `*storage_integration_name*`, where:
> 
...
> ```
> 
> If the S3 storage is in a public AWS region in China, use `'S3CHINA'` for the STORAGE\_PROVIDER parameter and `s3china://` protocol in STORAGE\_ALLOWED\_LOCATIONS.
...
> ```
> 
> If the S3 storage is in a public AWS region in China, use `'S3CHINA'` for the STORAGE\_PROVIDER parameter and `s3china://` protocol in STORAGE\_BLOCKED\_LOCATIONS.
