...
vendor: snowflake
topic: stream
fetched_at: 2026-06-10T06:43:37Z
revalidate_after: 2026-07-10T06:43:37Z
...
- [Directory tables](https://docs.snowflake.com/user-guide/data-load-dirtables)
- [Dynamic tables](https://docs.snowflake.com/user-guide/dynamic-tables/overview)
- [Apache Iceberg™ tables](https://docs.snowflake.com/user-guide/tables-iceberg) with [Limitations](#label-stream-limitations).
...
A new table version is created whenever a transaction that includes one or more [DML](https://docs.snowflake.com/sql-reference/sql-dml) statements is committed to the table. This applies to the following table types:
...
![Stream offset example](https://docs.snowflake.com/static/images/table-streams-offset.png)
...
Specifies a unique, immutable row ID for tracking changes over time. If CHANGE\_TRACKING is disabled and later re-enabled on the stream’s source object, the row ID could change.
...
1. The METADATA$ROW\_ID depends on the stream’s source object.
	For instance, a stream `stream1` on table `table1` and stream `stream2` on table `table1` produce the same METADATA$ROW\_IDs for the same rows, but a stream `stream_view` on view `view1` is not guaranteed to produce the same METADATA$ROW\_IDs as `stream1`, even if `view` is defined using the statement `CREATE VIEW view AS SELECT * FROM table1`.
2. A stream on a source object and a stream on the source object’s clone produce the same METADATA$ROW\_IDs for the rows that exist at the time of the cloning.
...
Overwritten or appended files are essentially handled as new files: The old version of the file is removed from cloud storage, but the insert-only stream does not record the delete operation. The new version of the file is added to cloud storage, and the insert-only stream records the rows as inserts. The stream does not record the diff of the old and new file versions. Note that appends may not trigger an automatic refresh of the external table metadata, such as when using [Azure AppendBlobs](https://docs.snowflake.com/user-guide/tables-external-azure).

## Data flow

The following diagram shows how the contents of a standard stream change as rows in the source table are updated. Whenever a DML statement consumes the stream contents, the stream position advances to track the next set of DML changes to the table (i.e. the changes in a [table version](#label-streams-table-versioning)):

![Streams Example](https://docs.snowflake.com/static/images/data-pipeline-streams.png)

Streams Example

## Data retention period and staleness

A stream becomes stale when its offset falls outside of the data retention period for its source table (or underlying tables for a source view). In a stale state, historical data and any unconsumed change records for the source table are no longer accessible. To continue tracking new change records, you must recreate the stream using the [CREATE STREAM](https://docs.snowflake.com/sql-reference/sql/create-stream) command.

To prevent a stream from becoming stale, consume the stream records within a DML statement during the table’s retention period and regularly consume its change data before its STALE\_AFTER timestamp (that is, within the extended data retention period for the source object). Additionally, calling [SYSTEM$STREAM\_HAS\_DATA](https://docs.snowflake.com/sql-reference/functions/system_stream_has_data) on the stream prevents it from becoming stale, provided the stream is empty and the SYSTEM$STREAM\_HAS\_DATA function returns `FALSE`.

For more information on data retention periods, see [Understanding & using Time Travel](https://docs.snowflake.com/user-guide/data-time-travel).

> [!note] Note
> Streams on shared tables or views don’t extend the data retention period for the table or underlying tables, respectively. For more information, see [Streams on shared objects](https://docs.snowflake.com/user-guide/data-sharing-provider#label-data-sharing-streams).

If the data retention period for a table is less than 14 days and a stream hasn’t been consumed, Snowflake temporarily extends this period to prevent the stream from going stale. The retention period is extended to the stream’s offset, up to a maximum of 14 days by default, regardless of your [Snowflake edition](https://docs.snowflake.com/user-guide/intro-editions). The maximum number of days for which Snowflake can extend the data retention period is determined by the [MAX\_DATA\_EXTENSION\_TIME\_IN\_DAYS](https://docs.snowflake.com/sql-reference/parameters#label-max-data-extension-time-in-days) parameter value. Once the stream is consumed, the extended data retention period reverts to the table’s default.

The following table shows examples of [DATA\_RETENTION\_TIME\_IN\_DAYS](https://docs.snowflake.com/sql-reference/parameters#label-data-retention-time-in-days) and MAX\_DATA\_EXTENSION\_TIME\_IN\_DAYS values, indicating how often the stream contents should be consumed to avoid staleness:
...
To check the staleness status of a stream, use the [DESCRIBE STREAM](https://docs.snowflake.com/sql-reference/sql/desc-stream) or [SHOW STREAMS](https://docs.snowflake.com/sql-reference/sql/show-streams) command. The STALE\_AFTER column timestamp is the extended data retention period for the source object. It shows when the stream is predicted to become stale or when it became stale if the timestamp is in the past. This timestamp is calculated by adding the greater value of the DATA\_RETENTION\_TIME\_IN\_DAYS or MAX\_DATA\_EXTENSION\_TIME\_IN\_DAYS parameters setting for the source object to the last consumption time of the stream.
...
Consuming change data for a stream updates the STALE\_AFTER timestamp. Although reading from the stream might succeed for some time after the STALE\_AFTER timestamp, the stream can become stale at any moment. The STALE column indicates if the stream is expected to be stale, though it might not be stale yet.

To prevent a stream from becoming stale, regularly consume its change data before its STALE\_AFTER timestamp (that is, within the extended data retention period for the source object). Don’t rely on the results from a stream after the STALE\_AFTER period has elapsed because the STREAM\_HAS\_DATA function might return unexpected results.

After the STALE\_AFTER timestamp has passed, the stream can become stale at any time, even if it has no unconsumed records. Querying a stream might return 0 records even if there is change data for the source object. For example, an append-only stream tracks row inserts only, but updates and deletes also write change records to the source object. Additionally, some table writes, like reclustering, don’t produce change data. Consuming change data for a stream advances its offset to the present, regardless of whether there is intervening change data.
...
Before creating a stream on a view, you must enable change tracking on the underlying tables for the view. For instructions, see [Enabling change tracking on views and underlying tables](https://docs.snowflake.com/user-guide/streams-manage#label-enabling-change-tracking-views).
...
As an alternative to streams, Snowflake supports querying change tracking metadata for tables or views using the [CHANGES](https://docs.snowflake.com/sql-reference/constructs/changes) clause for SELECT statements. The CHANGES clause enables querying change tracking metadata between two points in time without having to create a stream with an explicit transactional offset. Using the CHANGES clause does not advance the offset (i.e. consume the records). Multiple queries can retrieve the change tracking metadata between different transactional start and endpoints. This option requires specifying a transactional start point for the metadata using an [AT | BEFORE](https://docs.snowflake.com/sql-reference/constructs/at-before) clause; the end point for the change tracking interval can be set using the optional END clause.

A stream stores the current transactional [table version](#label-streams-table-versioning) and is the appropriate source of CDC records in most scenarios. For infrequent scenarios that require managing the offset for arbitrary periods of time, the CHANGES clause is available for your use.

Currently, the following must be true before change tracking metadata is recorded:

Tables:

Either enable change tracking on the table (using [ALTER TABLE](https://docs.snowflake.com/sql-reference/sql/alter-table) … CHANGE\_TRACKING = TRUE), or create a stream on the table (using [CREATE STREAM](https://docs.snowflake.com/sql-reference/sql/create-stream)).

Views:

Enable change tracking on the view and its underlying tables. For instructions, see [Enabling change tracking on views and underlying tables](https://docs.snowflake.com/user-guide/streams-manage#label-enabling-change-tracking-views).

Enabling change tracking adds several hidden columns to the table and begins storing change tracking metadata. The values in these hidden CDC data columns provide the input for the stream [metadata columns](#label-stream-metadata-columns). The columns consume a small amount of storage.
...
- When a [task is triggered](https://docs.snowflake.com/user-guide/tasks-triggered) by Streams on Views, then any changes to tables referenced by the Streams on Views query will also trigger the task, regardless of any joins, aggregations, or filters in the query.
