---
source_url: https://docs.snowflake.com/en/user-guide/views-secure
vendor: snowflake
topic: secure-views
fetched_at: 2026-06-10T06:39:56Z
revalidate_after: 2026-07-10T06:39:56Z
...
- For a non-secure view, the view definition is visible to other users.
	By default, the query expression used to create a standard view, also known as the view definition or text, is visible to users in various commands and interfaces. For details, see [Interacting with Secure Views](#interacting-with-secure-views) (in this topic).
...
Using the following widgets example, consider a user who has access to only the red widgets. Suppose the user wonders if any purple widgets exist and issues the following query:
...
- To create a secure view, specify the SECURE keyword in the [CREATE VIEW](https://docs.snowflake.com/sql-reference/sql/create-view) or [CREATE MATERIALIZED VIEW](https://docs.snowflake.com/sql-reference/sql/create-materialized-view) command.
- To convert an existing view to a secure view and back to a regular view, set/unset the SECURE keyword in the [ALTER VIEW](https://docs.snowflake.com/sql-reference/sql/alter-view) or [ALTER MATERIALIZED VIEW](https://docs.snowflake.com/sql-reference/sql/alter-materialized-view) command.

> [!note] Note
> In some cases, error messages related to secure views might be redacted. For more information, see [Secure objects: Redaction of information in error messages](https://docs.snowflake.com/release-notes/bcr-bundles/un-bundled/bcr-1858).
...
- [SHOW VIEWS](https://docs.snowflake.com/sql-reference/sql/show-views) and [SHOW MATERIALIZED VIEWS](https://docs.snowflake.com/sql-reference/sql/show-materialized-views) commands.
- [GET\_DDL](https://docs.snowflake.com/sql-reference/functions/get_ddl) utility function.
- [VIEWS](https://docs.snowflake.com/sql-reference/info-schema/views) Information Schema view.

However, users that have been granted IMPORTED PRIVILEGES privilege on the SNOWFLAKE database or another shared database have access to secure view definitions via the [VIEWS](https://docs.snowflake.com/sql-reference/account-usage/views) Account Usage view.
...
(For general information about the differences between INFORMATION\_SCHEMA views and ACCOUNT\_USAGE views, see [Differences between Account Usage and Information Schema](https://docs.snowflake.com/sql-reference/account-usage#label-differences-between-account-usage-and-information-schema).)
...
The internals of a secure view are not exposed in [Query Profile](https://docs.snowflake.com/user-guide/ui-snowsight-activity) (in the web interface). This is the case even for the owner of the secure view, because non-owners might have access to an owner’s Query Profile.

## Using Secure Views with Snowflake Access Control

View security can be integrated with Snowflake users and roles using the [CURRENT\_ROLE](https://docs.snowflake.com/sql-reference/functions/current_role) and [CURRENT\_USER](https://docs.snowflake.com/sql-reference/functions/current_user) context functions. The following example illustrates using roles to control access to the rows of a table. In addition to the table that contains the data (`widgets`), the example uses an access table (`widget_access_rules`) to track which roles have access to which rows in the data table:
...
The secure view’s WHERE clause is executed before any WHERE clause in the user’s query. Because purple widgets are excluded by the view, the user’s query never generates a division-by-zero error.

If the view were not secure, then the Snowflake optimizer could re-order the predicates in the WHERE clauses. This could allow the predicate in the user’s query to execute first, which would allow the division-by-zero error to occur.
...
- Use randomized identifiers (e.g. generated by [UUID\_STRING](https://docs.snowflake.com/sql-reference/functions/uuid_string)) instead of sequence-generated values.
...
When using secure views with [Secure Data Sharing](https://docs.snowflake.com/guides-overview-sharing), use the [CURRENT\_ACCOUNT](https://docs.snowflake.com/sql-reference/functions/current_account) function to authorize users from a specific account to access rows in a base table.

> > [!note] Note
> > When using the [CURRENT\_ROLE](https://docs.snowflake.com/sql-reference/functions/current_role) and [CURRENT\_USER](https://docs.snowflake.com/sql-reference/functions/current_user) functions with secure views that will be shared to other Snowflake accounts, Snowflake returns a NULL value for these functions. The reason is that the owner of the data being shared does not typically control the users or roles in the account with which the view is being shared.