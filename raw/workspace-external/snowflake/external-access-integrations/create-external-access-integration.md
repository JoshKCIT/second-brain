---
source_url: https://docs.snowflake.com/en/sql-reference/sql/create-external-access-integration
vendor: snowflake
topic: external-access-integrations
...
Creates an [external access integration](https://docs.snowflake.com/developer-guide/external-network-access/creating-using-external-network-access) for access to external network locations from a UDF or procedure handler.

See also:

[ALTER EXTERNAL ACCESS INTEGRATION](https://docs.snowflake.com/sql-reference/sql/alter-external-access-integration), [DROP INTEGRATION](https://docs.snowflake.com/sql-reference/sql/drop-integration), [SHOW INTEGRATIONS](https://docs.snowflake.com/sql-reference/sql/show-integrations), [DESCRIBE INTEGRATION](https://docs.snowflake.com/sql-reference/sql/desc-integration)
...
Specifies the allowed [network rules](https://docs.snowflake.com/sql-reference/sql/create-network-rule). Only egress rules may be specified.
...
Specifies the security integrations whose OAuth authorization server issued the secret used by the UDF or procedure. The security integration must be [the type used for external API integration](https://docs.snowflake.com/sql-reference/sql/create-security-integration-api-auth).
...
Security integrations specified by this parameter – as well as secrets specified by the ALLOWED\_AUTHENTICATION\_SECRETS parameter – are ways to allow secrets for use in a UDF or procedure that uses this external access integration. For more information, see [Usage notes](#label-create-external-access-integration-usage).

For reference information about security integrations, refer to [CREATE SECURITY INTEGRATION (External API Authentication)](https://docs.snowflake.com/sql-reference/sql/create-security-integration-api-auth).
...
The ALLOWED\_API\_AUTHENTICATION\_INTEGRATIONS parameter can also specify allowed secrets. For more information, see [Usage notes](#label-create-external-access-integration-usage).
...
- You can allow secrets for use by a UDF or procedure by using two external access integration parameters, as described below.
	- With the ALLOWED\_AUTHENTICATION\_SECRETS parameter. You can specify secrets as parameter values or set the parameter’s value to `all`, allowing handler code to use any secret.
		- With the ALLOWED\_API\_AUTHENTICATION\_INTEGRATIONS parameter. A [secret](https://docs.snowflake.com/sql-reference/sql/create-secret) is allowed for use when the secret itself specifies a security integration whose name is also specified by this parameter. The secret specifies the security integration with its API\_AUTHENTICATION parameter. In other words, when both the secret and the external access integration specify the security integration, the secret is allowed for use in functions and procedures that specify the external access integration.
...
For a more complete example, refer to [Creating and using an external access integration](https://docs.snowflake.com/developer-guide/external-network-access/creating-using-external-network-access).

1. Create a secret representing credentials.
	To create a secret, you must have been assigned a role with the CREATE SECRET privilege on the current schema. For other kinds of secret supported by this command, refer to [CREATE SECRET](https://docs.snowflake.com/sql-reference/sql/create-secret). In this example, `google_translate_oauth` refers to a security integration. For more information, refer to [CREATE SECURITY INTEGRATION (External API Authentication)](https://docs.snowflake.com/sql-reference/sql/create-security-integration-api-auth).
	```sql
...
2. Grant READ privileges on the secret to the `developer` role so that UDF developers can use it.
	...
	```
3. Create a network rule representing the external network location. Use a role with the privileges described in [CREATE NETWORK RULE](https://docs.snowflake.com/sql-reference/sql/create-network-rule).
	```sql
	USE ROLE SYSADMIN;
...
5. Grant USAGE privileges on the integration to the `developer` role so that UDF developers can use it.
	...
	```
6. Create a UDF `google_translate_python` that translates the specified text into a phrase in the specified language. For more information, refer to [Using the external access integration in a function or procedure](https://docs.snowflake.com/developer-guide/external-network-access/creating-using-external-network-access#label-creating-using-external-access-integration-using-udf).
	```sql
	USE ROLE developer;
