...
vendor: snowflake
topic: snowpipe-auto-s3
fetched_at: 2026-06-10T06:40:04Z
revalidate_after: 2026-07-10T06:40:04Z
...
Triggering automated Snowpipe data loads using S3 event messages is supported by Snowflake accounts hosted on [all supported cloud platforms](https://docs.snowflake.com/user-guide/intro-cloud-platforms).

## Network traffic

Note to [Virtual Private Snowflake (VPS)](https://docs.snowflake.com/user-guide/intro-editions) and [AWS PrivateLink](https://docs.snowflake.com/user-guide/admin-security-privatelink) customers:
...
> - Note that currently, accessing S3 storage in [government regions](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions) using a storage integration is limited to Snowflake accounts hosted on AWS in the same government region. Accessing your S3 storage from an account hosted outside of the government region using direct credentials is supported.

The following diagram shows the integration flow for a S3 stage:

![Amazon S3 Stage Integration Flow](https://docs.snowflake.com/static/images/storage-integration-s3.png)
...
> [!note] Note
> We highly recommend this option, which avoids the need to supply IAM credentials when accessing cloud storage. See [Configuring secure access to Amazon S3](https://docs.snowflake.com/user-guide/data-load-s3-config) for additional storage access options.
...
The following step-by-step instructions describe how to configure access permissions for Snowflake in your AWS Management Console to access your S3 bucket.

1. Log into the AWS Management Console.
2. From the home dashboard, search for and select **IAM**.
3. From the left-hand navigation pane, select **Account settings**.
4. Under **Security Token Service (STS)** in the **Endpoints** list, find the Snowflake [region](https://docs.snowflake.com/user-guide/intro-regions) where your account is located. If the **STS status** is inactive, move the toggle to **Active**.
...
8. Add a policy document that will allow Snowflake to access the S3 bucket and folder.
	...
	> - The Amazon Resource Names (ARN) for buckets in [government regions](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions) have a `arn:aws-us-gov:s3:::` prefix.
	```
	{
	 "Version": "2012-10-17",
	 "Statement": [
...
> ![Select trusted entity page in AWS Management Console](https://docs.snowflake.com/static/images/screens/iam-role-select-trusted-entity.png)
> 
...
6. Select the **Require external ID** option. An external ID is used to grant access to your AWS resources (such as S3 buckets) to a third party like Snowflake.
...
> ![Review Page in AWS Management Console](https://docs.snowflake.com/static/images/screens/iam-role-review.png)
> 
...
11. On the role summary page, locate and record the **Role ARN** value. In the next step, you will create a Snowflake integration that references this role.
...
- `bucket` is the name of a S3 bucket that stores your data files (for example, `mybucket`). The required STORAGE\_ALLOWED\_LOCATIONS parameter and optional STORAGE\_BLOCKED\_LOCATIONS parameter restrict or block access to these buckets, respectively, when stages that reference this integration are created or modified.
...
> [!note] Note
> Optionally, use the [STORAGE\_AWS\_EXTERNAL\_ID](https://docs.snowflake.com/sql-reference/sql/create-storage-integration#label-create-storage-integration-aws-ext-id) parameter to specify your own external ID. You might choose this option to use the same external ID across multiple external volumes and/or storage integrations.

## Step 4: Retrieve the AWS IAM user for your Snowflake account

1. To retrieve the ARN for the IAM user that was created automatically for your Snowflake account, use the [DESCRIBE INTEGRATION](https://docs.snowflake.com/sql-reference/sql/desc-integration).
	```
	DESC INTEGRATION <integration_name>;
	```
	Where:
	- `integration_name` is the name of the integration you created in [Step 3: Create a Cloud Storage Integration in Snowflake](#step-3-create-a-cloud-storage-integration-in-snowflake) (in this topic).
	For example:
	```sql
...
7. Modify the policy document with the DESC STORAGE INTEGRATION output values you recorded in [Step 4: Retrieve the AWS IAM user for your Snowflake account](#step-4-retrieve-the-aws-iam-user-for-your-snowflake-account) (in this topic):
...
> [!note] Note
> You can use the [SYSTEM$VALIDATE\_STORAGE\_INTEGRATION](https://docs.snowflake.com/sql-reference/functions/system_validate_storage_integration) function to validate the configuration for your storage integration.
...
- **Option 2. Existing event notification:** Configure [Amazon Simple Notification Service (SNS)](https://aws.amazon.com/sns/) as a broadcaster to share notifications for a given path with multiple endpoints (or “subscribers,” e.g. SQS queues or AWS Lambda workloads), including the Snowflake SQS queue for Snowpipe automation. An S3 event notification published by SNS informs Snowpipe via an SQS queue when files are ready to load.
	> [!note] Note
	> We recommend this option if you plan to use [Stage, pipe, and load history replication](https://docs.snowflake.com/user-guide/account-replication-stages-pipes-load-history). You can also migrate from option 1 to option 2 after you create a replication or failover group. For more information, see [Migrate to Amazon Simple Notification Service (SNS)](https://docs.snowflake.com/user-guide/account-replication-stages-pipes-load-history#label-account-replication-stages-pipes-load-history-migrate-to-sns).
...
![Snowpipe Auto-ingest Process Flow](https://docs.snowflake.com/static/images/data-load-snowpipe-s3-sqs.png)
...
Create an external stage that references your S3 bucket using the [CREATE STAGE](https://docs.snowflake.com/sql-reference/sql/create-stage) command. Snowpipe fetches your data files from the stage and temporarily queues them before loading them into your target table. Alternatively, you can use an existing external stage.
...
Create a pipe using the [CREATE PIPE](https://docs.snowflake.com/sql-reference/sql/create-pipe) command. The pipe defines the [COPY INTO <table>](https://docs.snowflake.com/sql-reference/sql/copy-into-table) statement used by Snowpipe to load data from the ingestion queue into the target table.
...
Use the [GRANT <privileges> … TO ROLE](https://docs.snowflake.com/sql-reference/sql/grant-privilege) command to grant privileges to the role.
...
To load any backlog of data files that existed in the external stage before SQS notifications were configured, see [Loading historic data](https://docs.snowflake.com/user-guide/data-load-snowpipe-manage#label-snowpipe-load-historic-data).

### Step 6: Delete staged files

Delete the staged files after you successfully load the data and no longer require the files. For instructions, see [Deleting staged files after Snowpipe loads the data](https://docs.snowflake.com/user-guide/data-load-snowpipe-manage#label-snowpipe-delete-data-files).
...
![Snowpipe Auto-ingest Process Flow with Amazon SNS](https://docs.snowflake.com/static/images/data-load-snowpipe-s3-sns.png)
...
5. Using a Snowflake client, query the [SYSTEM$GET\_AWS\_SNS\_IAM\_POLICY](https://docs.snowflake.com/sql-reference/functions/system_get_aws_sns_iam_policy) system function with your SNS topic ARN:
	> ```sql
...
Create an external stage that references your S3 bucket using the [CREATE STAGE](https://docs.snowflake.com/sql-reference/sql/create-stage) command. Snowpipe fetches your data files from the stage and temporarily queues them before loading them into your target table.
...
Create a pipe using the [CREATE PIPE](https://docs.snowflake.com/sql-reference/sql/create-pipe) command. The pipe defines the [COPY INTO <table>](https://docs.snowflake.com/sql-reference/sql/copy-into-table) statement used by Snowpipe to load data from the ingestion queue into the target table. In the COPY statement, identify the SNS topic ARN from [Prerequisite: Create an Amazon SNS Topic and Subscription](#prerequisite-create-an-amazon-sns-topic-and-subscription).
...
> 
> View the COPY INTO *<table>* statements in the definitions of all pipes in the account by executing [SHOW PIPES](https://docs.snowflake.com/sql-reference/sql/show-pipes) or by querying either the [PIPES](https://docs.snowflake.com/sql-reference/account-usage/pipes) view in Account Usage or the [PIPES](https://docs.snowflake.com/sql-reference/info-schema/pipes) view in the Information Schema.
...
The [SYSTEM$PIPE\_STATUS](https://docs.snowflake.com/sql-reference/functions/system_pipe_status) function retrieves a JSON representation of the current status of a pipe.
