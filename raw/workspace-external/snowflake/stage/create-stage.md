---
source_url: https://docs.snowflake.com/en/sql-reference/sql/create-stage
vendor: snowflake
topic: stage
fetched_at: 2026-06-10T06:43:33Z
revalidate_after: 2026-07-10T06:43:33Z
...
Stores data files internally within Snowflake. For more details, see [Choosing an internal stage for local files](https://docs.snowflake.com/user-guide/data-load-local-file-system-create-stage).
...
An internal or external stage can include a *directory table*. [Directory tables](https://docs.snowflake.com/user-guide/data-load-dirtables) store a catalog of staged files in cloud storage.

Additionally, this command supports the following variants:

- [Variant Syntax](#label-create-or-alter-stage-syntax): Creates a new stage if it doesn’t exist or alters an existing stage.
- [CREATE STAGE … CLONE](#label-create-stage-clone-syntax): Creates a clone of an existing stage. For more information, see [Cloning considerations](https://docs.snowflake.com/user-guide/object-clone).

See also:

[DROP STAGE](https://docs.snowflake.com/sql-reference/sql/drop-stage), [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage), [SHOW STAGES](https://docs.snowflake.com/sql-reference/sql/show-stages), [DESCRIBE STAGE](https://docs.snowflake.com/sql-reference/sql/desc-stage)

[PUT](https://docs.snowflake.com/sql-reference/sql/put), [COPY INTO <table>](https://docs.snowflake.com/sql-reference/sql/copy-into-table)

[COPY INTO <location>](https://docs.snowflake.com/sql-reference/sql/copy-into-location), [GET](https://docs.snowflake.com/sql-reference/sql/get), [CREATE OR ALTER <object>](https://docs.snowflake.com/sql-reference/sql/create-or-alter)
...
Creates a new stage if it doesn’t already exist, or transforms an existing stage into the stage defined in the statement. A CREATE OR ALTER STAGE statement follows the syntax rules of a CREATE STAGE statement and has the same limitations as an [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage) statement.
...
> [!note] Note
> When creating an external stage, a URL is also required. For more details, see [External Stage Parameters](#label-create-stage-externalstageparams) (in this topic).
> 
...
> 
> Specifies an existing named file format to use for the stage. The named file format determines the format type (CSV, JSON, etc.), as well as any other format options, for the data files loaded using this stage. For more details, see [CREATE FILE FORMAT](https://docs.snowflake.com/sql-reference/sql/create-file-format).
> 
> `TYPE = CSV | JSON | AVRO | ORC | PARQUET | XML | CUSTOM [ ... ]`
> 
> Specifies the type of files for the stage:
> 
> > - Loading data from a stage (using [COPY INTO <table>](https://docs.snowflake.com/sql-reference/sql/copy-into-table)) accommodates all of the supported format types.
> > - Unloading data into a stage (using [COPY INTO <location>](https://docs.snowflake.com/sql-reference/sql/copy-into-location)) accommodates `CSV`, `JSON`, or `PARQUET`.
> 
> If a file format type is specified, additional format-specific options can be specified. For more details, see [Format type options ()](#label-create-stage-formattypeoptions) (in this topic).
> 
...
- `SNOWFLAKE_FULL`: Client-side and server-side encryption. The files are encrypted by a client when it uploads them to the internal stage using [PUT](https://docs.snowflake.com/sql-reference/sql/put). Snowflake uses a 128-bit encryption key by default. You can configure a 256-bit key by setting the [CLIENT\_ENCRYPTION\_KEY\_SIZE](https://docs.snowflake.com/sql-reference/parameters#label-client-encryption-key-size) parameter.
...
- `SNOWFLAKE_SSE`: Server-side encryption only. The files are encrypted when they arrive on the stage by the cloud service where your Snowflake account is hosted.
	Specify server-side encryption if you plan to query pre-signed URLs for your staged files. For more information, see [Types of URLs available to access files](https://docs.snowflake.com/user-guide/unstructured-intro#label-unstructured-data-urls).
...
> 	- `s3gov` refers to S3 storage in [government regions](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions).
> 	Accessing cloud storage in a [government region](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions) using a storage integration is limited to Snowflake accounts hosted in the same government region.
...
> 	In these cases, use the CREDENTIALS parameter in the [CREATE STAGE](https://docs.snowflake.com/sql-reference/sql/create-stage) command (rather than using a storage integration) to provide the credentials for authentication.
> - `*bucket*` is the name of the S3 bucket or the [bucket-style alias](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-points-alias.html) for an S3 bucket access point. For an S3 access point, you must also specify a value for the `AWS_ACCESS_POINT_ARN` parameter.
> - `*path*` is an optional case-sensitive path for files in the cloud storage location (files have names that begin with a common string) that limits the set of files. Paths are alternatively called *prefixes* or *folders* by different cloud storage services.
> 
> `AWS_ACCESS_POINT_ARN = 'string'`
> 
> Specifies the Amazon resource name (ARN) for your S3 access point. Required only when you specify an S3 access point alias for your storage `URL`.
...
> 	Note that currently, accessing Azure blob storage in [government regions](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions) using a storage integration is limited to Snowflake accounts hosted on Azure in the same government region. Accessing your blob storage from an account hosted outside of the government region using direct credentials is supported.
...
> `STORAGE_INTEGRATION = *integration_name*`
> 
> Specifies the name of the storage integration used to delegate authentication responsibility for external cloud storage to a Snowflake identity and access management (IAM) entity. For more details, see [CREATE STORAGE INTEGRATION](https://docs.snowflake.com/sql-reference/sql/create-storage-integration).
> 
> > [!note] Note
...
> 
> `CREDENTIALS = ( AWS_KEY_ID = '*string*' AWS_SECRET_KEY = '*string*' [ AWS_TOKEN = '*string*' ] )` or  
> `CREDENTIALS = ( AWS_ROLE = '*string*' )`
> 
> Specifies the security credentials for connecting to AWS and accessing the private/protected S3 bucket where the files to load/unload are staged. For more information, see [Configuring secure access to Amazon S3](https://docs.snowflake.com/user-guide/data-load-s3-config).
> 
...
> > [!note] Note
> > - We highly recommend the use of storage integrations. This option avoids the need to supply cloud storage credentials using the CREDENTIALS parameter when creating stages or loading data.
> > - Accessing Azure blob storage in [government regions](https://docs.snowflake.com/user-guide/intro-regions#label-us-gov-regions) using a storage integration is limited to Snowflake accounts hosted on Azure in the same government region. Accessing your blob storage from an account hosted outside of the government region using direct credentials is supported.
> 
...
> > 
> > - `AWS_CSE`: Client-side encryption (requires a `MASTER_KEY` value). Currently, the client-side [master key](https://csrc.nist.gov/glossary/term/master_key) you provide can only be a symmetric key. When a `MASTER_KEY` value is provided, Snowflake assumes `TYPE = AWS_CSE` (when a `MASTER_KEY` value is provided, `TYPE` is not required).
...
> > 
> > For more information about the encryption types, see the AWS documentation for [client-side encryption](http://docs.aws.amazon.com/AmazonS3/latest/dev/UsingClientSideEncryption.html) or [server-side encryption](http://docs.aws.amazon.com/AmazonS3/latest/dev/serv-side-encryption.html).
> > 
> > - `NONE`: No encryption.
> > 
...
> > Specifies the encryption type used. Possible values are:
> > 
> > - `AZURE_CSE`: Client-side encryption (requires a MASTER\_KEY value). For information, see the [Client-side encryption information](https://docs.microsoft.com/en-us/azure/storage/common/storage-client-side-encryption) in the Microsoft Azure documentation.
> > - `NONE`: No encryption.
> > 
...
Specifies whether to use [private connectivity](https://docs.snowflake.com/user-guide/private-connectivity-outbound) for an external stage to harden your security posture.
...
- [Private connectivity to external stages for Amazon Web Services](https://docs.snowflake.com/user-guide/data-load-aws-private).
- [Private connectivity to external stages and Snowpipe automation for Microsoft Azure](https://docs.snowflake.com/user-guide/data-load-azure-private).
- [Private connectivity to external stages for Google Cloud](https://docs.snowflake.com/user-guide/data-load-gcs-private).
...
Specifies whether to enable a [directory table](https://docs.snowflake.com/user-guide/data-load-dirtables) on the internal named stage.

Default: `FALSE`

`AUTO_REFRESH = { TRUE | FALSE }`

Specifies whether Snowflake should automatically refresh the directory table metadata when new or updated data files are available on the [internal named stage](https://docs.snowflake.com/user-guide/data-load-local-file-system-create-stage#label-data-load-local-file-system-table-stage-named-stages).
...
Snowflake does not automatically refresh the directory table metadata. You must manually refresh the directory table metadata periodically using [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage) … REFRESH to synchronize the metadata with the current list of files in the stage path.
...
> > `ENABLE = { TRUE | FALSE }`
> > 
> > Specifies whether to add a [directory table](https://docs.snowflake.com/user-guide/data-load-dirtables) to the stage. When the value is TRUE, a directory table is created with the stage.
> > 
> > Default: `FALSE`
> > 
> > `REFRESH_ON_CREATE = { TRUE | FALSE }`
...
> > `FALSE`
> > 
> > Snowflake does not automatically refresh the directory table metadata. To register any data files that exist in the stage, you must manually refresh the directory table metadata once using [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage) … REFRESH.
> > 
> > Default: `TRUE`
> > 
> > `AUTO_REFRESH = { TRUE | FALSE }`
...
> [!note] Note
> If the `SINGLE` copy option is `TRUE`, then the COPY command unloads a file without a file extension by default. To specify a file extension, provide a file name and extension in the `internal_location` or `external_location` path (For example, `copy into @stage/data.csv`).
...
Defines the format of date values in the data files (data loading) or table (data unloading). If a value is not specified or is `AUTO`, the value for the [DATE\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-date-input-format) (data loading) or [DATE\_OUTPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-date-output-format) (data unloading) parameter is used.
...
Defines the format of time values in the data files (data loading) or table (data unloading). If a value is not specified or is `AUTO`, the value for the [TIME\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-time-input-format) (data loading) or [TIME\_OUTPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-time-output-format) (data unloading) parameter is used.
...
Defines the format of timestamp values in the data files (data loading) or table (data unloading). If a value is not specified or is `AUTO`, the value for the [TIMESTAMP\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-timestamp-input-format) (data loading) or [TIMESTAMP\_OUTPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-timestamp-output-format) (data unloading) parameter is used.
...
> [!note] Note
> When [transforming data during loading](https://docs.snowflake.com/user-guide/data-load-transform) (i.e. using a query as the source for the COPY command), this option is ignored. There is no requirement for your data files to have the same number and ordering of columns as your target table.
...
Defines the format of date string values in the data files. If a value is not specified or is `AUTO`, the value for the [DATE\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-date-input-format) parameter is used.
...
Defines the format of time string values in the data files. If a value is not specified or is `AUTO`, the value for the [TIME\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-time-input-format) parameter is used.
...
Defines the format of timestamp string values in the data files. If a value is not specified or is `AUTO`, the value for the [TIMESTAMP\_INPUT\_FORMAT](https://docs.snowflake.com/sql-reference/parameters#label-timestamp-input-format) parameter is used.
...
This file format option is applied to the following actions only when loading JSON data into separate columns using the MATCH\_BY\_COLUMN\_NAME copy option.
...
This file format option is applied to the following actions only when loading Avro data into separate columns using the MATCH\_BY\_COLUMN\_NAME copy option.
...
This file format option is applied to the following actions only when loading Orc data into separate columns using the MATCH\_BY\_COLUMN\_NAME copy option.
...
This file format option is applied to the following actions only when loading Parquet data into separate columns using the MATCH\_BY\_COLUMN\_NAME copy option.
...
> 	If you must recreate a stage after it has been linked to one or more external tables, you must recreate each of the external tables (using CREATE OR REPLACE EXTERNAL TABLE) to reestablish the association. Call the [GET\_DDL](https://docs.snowflake.com/sql-reference/functions/get_ddl) function to retrieve a DDL statement to recreate each of the external tables.
...
- CREATE STAGE doesn’t check whether the specified URL or credentials are valid. If the credentials aren’t valid, when you attempt to use the stage, the system returns an error.
...
- For external stages that use an S3 access point:
	- If you’re using a storage integration, you must configure the IAM policy for the integration to grant permission to your S3 access point. For more information, see [Option 1: Configure a Snowflake storage integration to access Amazon S3](https://docs.snowflake.com/user-guide/data-load-s3-config-storage-integration).
...
- All limitations of the [ALTER STAGE](https://docs.snowflake.com/sql-reference/sql/alter-stage) command apply.
...
- The CREATE OR ALTER STAGE command does not support refreshing directory tables.
	- Newly created directory tables will not be refreshed.
		- To refresh a directory table use [ALTER REFRESH](https://docs.snowflake.com/sql-reference/sql/alter-stage#label-alter-stage-directory-table-syntax).
...
Create a temporary internal stage that references a file format named `my_csv_format` (created using [CREATE FILE FORMAT](https://docs.snowflake.com/sql-reference/sql/create-file-format)):
...
When you reference the stage in a [COPY INTO <table>](https://docs.snowflake.com/sql-reference/sql/copy-into-table) statement, the file format options are automatically set.

Create an internal stage that includes a [directory table](https://docs.snowflake.com/user-guide/data-load-dirtables). The stage references a file format named `myformat`:
...
> ```
> 
> Create an external stage using an S3 bucket named `load` with a folder path named `encrypted_files` and AWS\_SSE\_KMS server-side encryption with the ID for the master key to decrypt/encrypt files stored in the bucket:
> 
> ```sql
