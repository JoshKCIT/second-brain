...
- Created: wiki/projects/{slug}/{subdirs}
- Technical: {true|false}
...
Invoke `.github/prompts/vp-agent.prompt.md` with the captured intent. Pass:
...
The VP agent produces `wiki/projects/{slug}/01-vp-brief/product-brief.md` at status `draft`.
...
Invoke `.github/prompts/pm-agent.prompt.md` with the approved VP brief.

Output: `wiki/projects/{slug}/02-pm-prd/product-requirements.md` at status `draft`.
...
Invoke `.github/prompts/architect-agent.prompt.md` with the approved PRD.

Output: `wiki/projects/{slug}/03-architecture/architectural-approaches.md` at status `draft`. May also produce ADRs in `wiki/projects/{slug}/03-architecture/adrs/` for specific decisions.
...
Invoke `.github/prompts/engineer-agent.prompt.md` with the approved architecture doc (or PRD if non-technical).

Output: `wiki/projects/{slug}/04-engineering/{spec-files}.md` at status `draft`.

## Step 11: Finalize

Run the finalize step (Engineer-led):

- For every artifact in `wiki/projects/{slug}/`:
...
- `.github/prompts/align-cite.prompt.md` against the active set
- `.github/prompts/align-closure.prompt.md` against the active set
...
Tell the CEO: project is at `review` status; run `/publish` when ready, or run additional align checks (`align-conformance`, `align-coverage`, `align-vendor-truth`) for deeper verification before publish.

## Resumability

If invoked again on a project that already has artifacts (detected by `wiki/projects/{slug}/meta.yml` exists), resume at the next incomplete stage rather than starting over. Read the existing artifacts and `meta.yml` to determine where to pick up.
