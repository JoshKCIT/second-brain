...
- Project slug
- The approved PRD at `wiki/projects/{slug}/02-pm-prd/product-requirements.md`
- The VP brief at `wiki/projects/{slug}/01-vp-brief/product-brief.md`
- Project meta.yml

You read:

- PRD (input contract)
- In-scope wiki standards (especially `wiki/standards/architecture/`, `wiki/standards/security/`, `wiki/standards/data-modeling/`, etc.)
- Relevant `wiki/concepts/` for system patterns
- Cached vendor docs in `raw/external/` for vendor capability validation
- Past architecture docs from `wiki/projects/*/03-architecture/` (only those at status `published` or `archived`; per cross-project rule, in-progress projects are not referenceable)

## Output

Primary: `wiki/projects/{slug}/03-architecture/architectural-approaches.md`

Optional: ADRs at `wiki/projects/{slug}/03-architecture/adrs/{NNNN-slug}.md` for specific decisions. Use the `adr-generator` agent (`.github/agents/adr-generator.agent.md`) when invoking ADR creation.
...
sources:
  - "wiki/projects/{slug}/02-pm-prd/product-requirements.md"
  - {wiki/standards/ and raw/external/ paths}
created: {ISO date}
updated: {ISO date}
---
...
- Invoke `.github/agents/adr-generator.agent.md` with the decision context
- ADR files land in `wiki/projects/{slug}/03-architecture/adrs/{NNNN-decision-slug}.md`
...
3. Append to `wiki/log.md`:
   ...
   - Output: wiki/projects/{slug}/03-architecture/architectural-approaches.md
