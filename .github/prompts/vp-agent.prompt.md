...
- In-scope spaces (from `config/second-brain.yml` plus any project-specific additions in `wiki/projects/{slug}/meta.yml`)

You may also read the wiki for context (e.g., `wiki/standards/strategy/`, `wiki/concepts/{relevant topics}`).

## Output

Write to: `wiki/projects/{slug}/01-vp-brief/product-brief.md`
...
- **Internal standards from in-scope spaces:** cite as wikilinks to `wiki/standards/{team}/{slug}` if the standard exists in the wiki. If a relevant standard is not yet in the wiki but exists in `raw/confluence/`, tell the CEO and offer to compile it before proceeding.
- **Vendor sources:** generally not relevant at this stage. If a vendor capability is named in the CEO's intent (e.g., "we will use Snowflake"), cite parenthetically with `(per Snowflake docs)` and add a See Also link if a cached vendor doc exists in `raw/external/snowflake/`. Otherwise note in Open Questions: "Need to verify Snowflake capability for X."
...
2. Append to `wiki/log.md`:
   ...
   - Output: wiki/projects/{slug}/01-vp-brief/product-brief.md
