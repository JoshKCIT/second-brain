---
description: Ingest Confluence pages via the user's existing API-based skill. Per-page output to raw/confluence/{space-key}/pages/, synchronous compile to wiki/, post-ingest manifest with quarantine for failures.
mode: agent
---

# /ingest-confluence

You are ingesting Confluence pages into the user's Second Brain. The user provides a space key, page ID, page URL, or list of these. Output lands in `raw/confluence/{space-key}/pages/{page-id}--{slug}.md`. The wiki layer is compiled synchronously.
...
- Verify `raw/confluence/{space-key}/` exists; create if not.
...
4. **Write to raw.** `raw/confluence/{space-key}/pages/{page-id}--{slug}.md`.
...
After the raw page lands, invoke `compile.prompt.md` to integrate it into the wiki layer. Compile is part of ingest; do not return until both raw and wiki are updated.
...
If the user opts for a smoke test, run `query.prompt.md` against a few canned queries derived from the new content's titles or labels.
