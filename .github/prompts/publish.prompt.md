...
1. `align-cite.prompt.md` against the artifact(s)
2. `align-closure.prompt.md` against the project
...
- **review** → invoke `prepare-for-confluence.prompt.md`
- **confluence** → invoke `publish-to-confluence.prompt.md`
...
- For Confluence: transition status to `published` on every published artifact; update `wiki/projects/{slug}/meta.yml` `status: published`
