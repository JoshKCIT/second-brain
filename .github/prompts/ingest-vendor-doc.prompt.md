---
description: Fetch a vendor doc on demand using the defuddle skill, cache to raw/external/ with TTL frontmatter for revalidation.
mode: agent
---
...
Check `raw/external/{vendor}/{topic}/` for existing cached entry:
...
4. Write to `raw/external/{vendor}/{topic}/{slug}.md` where `{slug}` is derived from the URL or topic.
...
6. Optionally compile into the wiki layer if the cached doc warrants a `wiki/concepts/` or `wiki/standards/{vendor}/` article. For simple capability lookups (e.g., "does S3 support SSE-KMS"), the raw cache is sufficient.
