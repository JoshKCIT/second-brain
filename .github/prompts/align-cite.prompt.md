...
- Path to one artifact, OR a project slug (in which case run on every artifact in `wiki/projects/{slug}/`)
...
- External URLs: do not fetch live; trust the cached `raw/external/` if it exists, otherwise mark as PLAUSIBLE with note "external source not cached locally"
- Confluence URLs (when source is `raw/confluence/{space}/pages/.../`): treat the `raw/` cache as the source of truth
...
Write a structured report to `reports/align-cite-{artifact-slug}-{date}.md`:
