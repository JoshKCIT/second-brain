...
- Project slug
- For technical projects: `wiki/projects/{slug}/03-architecture/architectural-approaches.md` and any ADRs
- The PRD at `wiki/projects/{slug}/02-pm-prd/product-requirements.md`
- The VP brief
- Project meta.yml

You read all of the above plus:

- Cached vendor docs in `raw/external/` for capability details referenced in the architecture
...
Primary: one or more files in `wiki/projects/{slug}/04-engineering/`. Decompose by major component or subsystem; do not produce a single 50-page monolithic spec. Suggested decomposition:
...
sources:
  - "wiki/projects/{slug}/03-architecture/architectural-approaches.md"
...
For each `*.md` file in `wiki/projects/{slug}/`:
...
3. **For each body wikilink:**
   ...
   - If it points to a vendor doc cached in raw/external/: change the prose to use parenthetical attribution; move the link to See Also (use the source_url from frontmatter)
