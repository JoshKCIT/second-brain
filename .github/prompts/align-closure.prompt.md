...
- Project slug (run on every artifact in `wiki/projects/{slug}/`)
...
- If it points to another project (`wiki/projects/{other-slug}/...`), check the other project's status:
  - **Pass:** other project is `published`
  - **Fail:** other project is `in-progress` (any sub-status) or `archived`
- If it points to wiki/standards, wiki/recommendations, wiki/informational, wiki/concepts, wiki/connections: pass (these are referenceable regardless)
...
3. **Restate:** rewrite this artifact to not depend on the other project (inline the needed content from the other project's published artifacts, or extract a `wiki/concepts/` article)
...
Write a structured report to `reports/align-closure-{slug}-{date}.md`:
