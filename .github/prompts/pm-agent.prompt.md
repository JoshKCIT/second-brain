...
- Project slug
- The approved VP brief at `wiki/projects/{slug}/01-vp-brief/product-brief.md`
- Project meta.yml (in-scope spaces, alignment defaults)

You read:

- The VP brief (input contract)
- In-scope wiki standards (`wiki/standards/{team}/`) and recommendations
- Relevant `wiki/concepts/` articles
- Cached vendor docs in `raw/external/` if vendor capabilities are named

## Output

Write to: `wiki/projects/{slug}/02-pm-prd/product-requirements.md`
...
sources:
  - "wiki/projects/{slug}/01-vp-brief/product-brief.md"
...
- **Vendor capability claims:** parenthetical attribution + See Also link to cached vendor doc in `raw/external/{vendor}/`. If no cache exists for a relevant vendor capability, surface in Open Questions: "Need to ingest vendor doc for X to verify capability claim."
...
2. Append to `wiki/log.md`:
   ...
   - Output: wiki/projects/{slug}/02-pm-prd/product-requirements.md
