---
description: Index-guided query against the wiki. Reads index.md and Base views, identifies relevant articles, synthesizes an answer with citations. Optional file-back saves the answer as a wiki/qa/ article.
mode: agent
---
...
- Optional `--file-back` flag: persist the answer as a `wiki/qa/` article
...
- `wiki/standards/` (highest authority)
- `wiki/recommendations/`
- `wiki/concepts/`
- `wiki/connections/` (cross-cutting)
- `wiki/qa/` (prior answers)

### Step 3: Filter by scope (if applicable)

If `--project` is set, filter to articles whose sources are in the project's `in_scope_spaces` (per `wiki/projects/{slug}/meta.yml`).

If `--include-archives` is not set, exclude any article with `archived: true` in frontmatter or under `wiki/archives/`.
...
- Cites every fact-bearing claim to its wiki source (using section anchors where possible) or to a raw/external/ vendor cache
- Uses the authority+domain rule for conflicts: vendor truth for vendor claims; internal standards for internal architecture
- If a vendor capability is cited but no cached vendor doc exists in `raw/external/`, offer to ingest it via `/ingest-vendor-doc` before continuing
...
If `--file-back` is set, save the answer as `wiki/qa/{slug}.md`:
