...
- `jq` for JSON processing (pre-installed on most CI environments and macOS). **Recommended:** install `jq` so audit logs include full structured payloads (threat arrays, jq-native field quoting).
- **Without `jq`:** hooks still run; session start/end and clean prompts append NDJSON via shell `printf` with escaping; prompt parsing falls back to raw input instead of JSON fields; when threats are detected without `jq`, a summary line is logged (`detail` explains reduced logging).
