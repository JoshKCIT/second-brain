...
if [[ ${#THREATS_FOUND[@]} -gt 0 ]]; then
  ...
  if have_jq; then
    ...
    printf '{"timestamp":"%s","event":"threat_detected","governance_level":"%s","threat_count":%s,"max_severity":%s,"threats":[],"detail":"install jq for per-threat structured logs in audit.log"}\n' \
      "$(json_escape "$TIMESTAMP")" "$(json_escape "$LEVEL")" "${#THREATS_FOUND[@]}" "$MAX_SEVERITY" >> "$LOG_FILE"
  fi

  echo "⚠️ Governance: ${#THREATS_FOUND[@]} threat signal(s) detected (max severity: $MAX_SEVERITY)"
  for threat in "${THREATS_FOUND[@]}"; do
    IFS=$'\t' read -r category severity description __evidence_encoded <<< "$threat"
  ...
  if have_jq; then
    ...
    printf '{"timestamp":"%s","event":"prompt_scanned","governance_level":"%s","status":"clean"}\n' \
      "$(json_escape "$TIMESTAMP")" "$(json_escape "$LEVEL")" >> "$LOG_FILE"
  fi
fi
